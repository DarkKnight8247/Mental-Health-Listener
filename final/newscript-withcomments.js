  (() => {
      ...
      // Append message: user instantly, bot with typing animation
      function appendMessage(text, sender) {
          ...
          if (sender === 'user') {
              ...
              msgEl.textContent = ''; // start empty
              let i = 0;
              const typingInterval = setInterval(() => {
                  if (i < text.length) {
                      msgEl.textContent += text.charAt(i);
                      i++;
                      chat.scrollTop = chat.scrollHeight; // keep scrolling as it types
                  } else {
      ...
      function renderConversation(conversation) {
          ...
          conversation.forEach(({ text, sender }) => {
              const msgEl = document.createElement('div');
              msgEl.className = 'message ' + sender;
              msgEl.textContent = text; // instant, no animation
              chat.appendChild(msgEl);
          });
          chat.scrollTop = chat.scrollHeight;
      }
      ...
      async function sendQuestion(question, lang) {
          appendMessage(question, 'user');  // user message instantly
          ...
          try {
              ...
              appendMessage(botResponse, 'bot');  // bot with animation
